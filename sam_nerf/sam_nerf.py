...
class SAMNerfModel(Model):
    ...
    config: ClipNerfModelConfig
    field: ClipNerfField

    def __init__(self, config: ClipNerfModelConfig, metadata: Dict, **kwargs) -> None:
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.training:
            outputs["clip"] = self.renderer_clip(
                    embeds=field_outputs[ClipFieldHeadNames.CLIP], weights=weights.detach(), ray_indices=ray_indices, num_rays=num_rays
                ) #weights.detach()
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        #     t = {}
        #     i = -1
        #     for n, k, r in zip(batch['indices'], batch["clip"].to(self.device), outputs["clip"].detach()):
        #         i+=1
        #         if int(n[0]) not in t or i%9==0:
        #             t[int(n[0])] = torch.stack([k, r]).unsqueeze(0)
        #         else:
        #             t[int(n[0])] = torch.cat((t[int(n[0])], torch.stack([k, r]).unsqueeze(0)), 0)         
        #     loss = []
        #     for k, v in t.items():
        #         v[:,0] /= (v[:,0].norm(dim=-1, keepdim=True)+0.000001)
        #         res = torch.mm(v[:, 0],v[:, 0].T)
        #         mask = res[4, :] > 0.99
        #         loss.append(abs(v[:, 0] - v[:, 1])[mask].nanmean()) 
            
        #     loss_dict["reg_loss"] = torch.tensor(self.config.reg_loss_weight * torch.tensor(loss).nanmean())#.clone().detach()
